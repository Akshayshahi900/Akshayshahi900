<h1 align="center">Hi 👋, I'm Akshay Shahi</h1>
<h3 align="center">Fullstack • Backend Systems • Infrastructure </h3>

<p align="center">
I build software from the operating system upwards — reverse proxies, in-memory databases, networking layers, GEN AI applications and scalable backend services.
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/akshay-shahi-5815602a7"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="mailto:shahiakshay900@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://akshayshahi.vercel.app"><img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" /></a>
</p>

---

### 🚀 About Me

- 💻 Backend & Systems Engineering enthusiast
- ⚙️ I like building infrastructure from scratch instead of relying on black-box libraries
- 🧠 Interested in how Linux, networking, databases, and distributed systems actually work
- 🌱 Currently learning **Rust, Distributed Systems & Cloud Infrastructure**
- 🎯 Open to **Fullstack, Backend Systems & Infrastructure** roles
- 🏆 National Finalist — HCL × GUVI India AI Impact Buildathon 2026 (Top 200 of 15,000+ teams)

---

### 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=cpp,rust,ts,js,py,c" />
</p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=nodejs,express,postgres,mongodb,prisma,redis" />
</p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=nextjs,react,tailwind,redux" />
</p>
<p align="center">
  <img src="https://skillicons.dev/icons?i=docker,git,githubactions,linux,vercel,render" />
</p>

---

### 📌 Featured Projects

| Project | Description | Link |
|---|---|---|
| 🧵 **Thread_pool** | C++20 fixed-size thread pool with work-stealing & future-based async execution | [Repo](https://github.com/Akshayshahi900/Thread_pool) |
| ⚡ **hy-proxy** | Epoll-based async reverse proxy in C++ with round-robin load balancing | [Repo](https://github.com/Akshayshahi900/hy-proxy) |
| 🗄️ **Voidstore** | Redis-like in-memory DB in C++ with RESP protocol, TTL & AOF persistence | [Repo](https://github.com/Akshayshahi900/Voidstore) |
| 🤖 **SaffronAI** | AI honeypot API using LLMs to engage scam callers & extract intel | [Repo](https://github.com/Akshayshahi900/SaffronAI) |
| 💬 **chat-app** | Real-time chat platform with WebSockets & secure auth | [Repo](https://github.com/Akshayshahi900/chat-app) |
| 💰 **Outmoni** | Full-stack finance tracking SaaS with JWT auth, Docker & Prisma | [Repo](https://github.com/Akshayshahi900/Outmoni) |

---

### 📊 GitHub Stats

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=Akshayshahi900&show_icons=true&theme=tokyonight&count_private=true" />
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Akshayshahi900&layout=compact&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com/?user=Akshayshahi900&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Akshayshahi900&theme=tokyo-night" />
</p>

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Akshayshahi900&theme=tokyonight&no-frame=true&row=1&column=6" />
</p>

---

<p align="center"><i>"I like understanding systems from first principles and building them from scratch."</i></p>
